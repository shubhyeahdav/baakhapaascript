...
from database import (
    supabase, get_project_by_id, get_versions_by_script, get_scene_by_id,
)
...
# Which scenes carry the turning points. The writer's own answer, because
# nothing else in the system can have one: a generated structure guesses, and a
# scene typed by hand is created `minor` by default and had no way to become
# anything else.
SCENE_TYPES = {"major", "minor"}

# Deliberately only `scene_type`. `title` looks like the obvious second entry
# and must not be: a scene's title IS its slugline in the draft, and
# `scene_sync` derives the row from the page on every save. Setting it here
# would be overwritten by the next keystroke, which is worse than not offering
# it — the editor renames a scene by rewriting the line, and that is the only
# way a rename survives.
SCENE_UPDATE_FIELDS = {"scene_type"}


@router.put("/scenes/{scene_id}")
def update_scene(scene_id: str, updates: dict, user_id: str = Depends(get_current_user)):
    """Mark a scene as a turning point, or stop marking it as one.

    The product asks a writer to think in majors and minors — the scene rail
    counts them, the outline reads act balance from them, and the storyboard
    picks a shot type from them — and until now nothing could set one. A
    structure generated by a model chose, once, and a scene the writer typed
    themselves was `minor` for ever. So the one person who actually knows which
    scene is the turning point was the only one who could not say so.

    `scene_sync` never touches this field, which is what makes it safe to store
    on the row: the draft is the authority on order, location and heading, and
    the writer is the authority on weight.
    """
    scene = get_scene_by_id(scene_id)
    if not scene:
        # 404 rather than 403, and the same 404 a stranger gets, so the route
        # cannot be used to discover which scene ids exist.
        raise HTTPException(status_code=404, detail="Scene not found")

    require_script_access(scene["script_id"], user_id)
    safe = apply_whitelist(updates, SCENE_UPDATE_FIELDS)

    kind = safe.get("scene_type")
    if kind is not None and kind not in SCENE_TYPES:
        raise HTTPException(
            status_code=400,
            detail=f"scene_type must be one of {sorted(SCENE_TYPES)}",
        )

    result = supabase.table("scenes").update(safe).eq("id", scene_id).execute()
